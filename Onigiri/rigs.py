...
def get_associated_mesh(arm, report=False):
    ...
    if isinstance(arm, str):
        if utils.is_valid(arm):
            ...
            if report == True:
                print(
                    "rigs::get_associated_mesh reports : the armature is not in the scene",
                    arm,
                )
            return False
    ...
    for cObj in armObj.children:
        if cObj.type != "MESH":
            if report == True:
                print(
                    "rigs::get_associated_mesh reports : child object is not a mesh",
                    cObj.name,
                )
            continue
        mesh.append(cObj)
...
def make_complete(
    ...
    if len(bad_bones) == len(armObj.data.bones):
        print(
            "rigs::make_complete : None of the bones in the rig matched, there's nothing to do"
        )
