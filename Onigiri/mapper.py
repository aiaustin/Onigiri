...
def mapper_init():
    ...
    obj[bmp.mapper_source_name]["location"] = obj[
...
def mapper_handler(context):
    ...
    if bmp.mapper_source_name == "":
        ...
        print(
            "mapper_handler reports: terminating mapper, source became empty while working:",
            bmp.mapper_source_name_backup,
        )
    ...
    if bmp.mapper_source_name not in obj:
        ...
        bmp.mapper_enabled = False
        print(
            "mapper_handler reports: terminating mapper, source removed:",
            bmp.mapper_source_name_backup,
        )
        bpy.ops.onigiri.mapper_reset()
        return
...
def store_bone_map(source="", sbone="", target="", tbone=""):
    if source == "" or target == "" or sbone == "" or tbone == "":
        print(
            "something is missing, recheck your functions: source/sbone/target/tbone - ",
            source,
            sbone,
            target,
            tbone,
...
def restore_rig(armature="", type="", roll=False, data="all"):
    ...
    if error == 1:
        print(
            'usage: restore_rig(armature="someArmatureName", type="[pose/edit]", [roll=True (default = False)]'
        )
        return
    ...
    if obj[armature].type != "ARMATURE":
        print(
            "restore_rig reports: the object sent as an armature is NOT an armature",
            armature,
        )
        return False
...
def restore_bone(armature="", bone="", type="", roll=False):
    ...
    if error == 1:
        print(
            'usage: restore_bone(armature="someArmatureName", bone="someBoneName, type="[pose/edit]", [roll=True (default = False)]'
        )
        return False
    if armature not in obj:
    ...
    if obj[armature].type != "ARMATURE":
        print(
            "restore_bone reports: the object sent as an armature is NOT an armature",
            armature,
        )
        return False

    if type == "pose":
        obj[armature].data.edit_bones[bone].head = obj[armature]["bone_data"][bone][
            "pose"
        ]["head"]
...
def snap_to(source=""):
    ...
    if 1 == 0:
        ...
        for sbone in bone_map:
            if sbone not in obj[source].data.bones:
                print(
                    "Missing source bone in target armature, this is not necessarily bad, you might have a control rig:",
                    sbone,
                )
            ...
            obj[sarm].pose.bones[sbone].matrix = (
                obj[tarm].pose.bones[tbone].matrix.copy()
                @ obj[tarm].matrix_world.inverted()
            )
            bpy.context.view_layer.update()
    ...
    if 1 == 1:
        ...
        for sbone in bone_map:
            ...
            tarm_matrix_head = (
                obj[tarm].matrix_world @ obj[tarm].pose.bones[tbone].head.copy()
            )
            tarm_matrix_tail = (
                obj[tarm].matrix_world @ obj[tarm].pose.bones[tbone].tail.copy()
            )
            ...
            obj[source].data.edit_bones[sbone].roll = obj[tarm]["bone_data"][tbone][
                "edit"
            ]["roll"]
    ...
    try:
        bpy.context.window_manager.oni_mapper.mapper_message = mapper_message = (
            "Use (Attach) if you are animating"
        )
    except:
