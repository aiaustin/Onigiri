...
def create_rig(target="default"):
    ...
    for bone in rig_target:
        armObj.pose.bones[bone].matrix = mathutils.Matrix(
            rig_target[bone]["matrix_edit"]
        )
    ...
    for bone in rig_target:
        if rig_target[bone]["type"] == "bone":
            ...
            palette = palette = mod_data.rig_group_mtheme
        else:
            group = mod_data.rig_group_vbones
            palette = palette = mod_data.rig_group_vtheme
...
def force_rig_to_class(armature="", rig_class=""):
    ...
    if rig_class == "default":
        ...
        popup(
            "This is a bug, please report it, check console for details",
            "Error",
            "ERROR",
        )
        return False
    ...
    for boneObj in armObj.data.edit_bones:
        ...
        if name in skel.avatar_skeleton:
            ...
            armObj.data.edit_bones[bone].matrix = mathutils.Matrix(
                rig_target[name]["matrix_edit"]
            )
...
def convert_rig_to_class(armature=None, rig_class=None, brand="onigiri"):
    ...
    if 1 == 0:
        ...
        print(
            "There's no reason to attempt to confert this to a kit compatible rig when it's not complete."
        )
    ...
    if 1 == 0:
        ...
        if bone not in skel.avatar_skeleton:
            print(
                "A bone was encountered that's used for controlling weights and is not part of the SL definition:",
                bone,
            )
            print("It makes no sense to attempt to make this rig compatible with a kit")
            popup(
                "Incompatible bone found in rig, see System Console", "Error", "ERROR"
            )
            return False
...
def restore_hide_state(state):
    ...
    for bone in onim["hide_states"][state]["hide"]:
        ...
        armObj.data.bones[bone].hide_select = onim["hide_states"][state]["hide_select"][
            bone
        ]
...
def rebind(arm, keep_animation=False, report=False):
    ...
    print(
        "rigutils::rebind reports - animation now travels with the rebind if (keep_animation=True), re-examine this for issues"
    )
        ...
        for modObj in meshObj.modifiers:
            ...
            try:
                ...
                if report:
                    ...
                    print("B28-Failed!")
                    print(
                        "B29-rigs::rebind[Attempting to apply modifiers]", meshObj.name
                    )
                try:
    ...
    if len(select_error) > 0:
        if report:
            ...
            pop_text += (
                "to see which objects are affected by this.  If you see an excessive\n"
            )
            ...
            pop_text += (
                "the (rebind) again and note the errors produced.  Unhide and make\n"
            )
            pop_text += (
                "selectable the indicated objects in order to proceed.  It's possible\n"
            )
            ...
            pop_text += (
                "attempt.  Noting this you'll want to load a known good file, do not\n"
            )
...
def reset_pose(
    armature=None, location=False, rotation=True, scale=False, space="rig", bone=None
):
    obj = bpy.data.objects
    arm = armature
...
def attach_proxy_rig(armature=None, clean=False):
    ...
    for b in obj[glue].pose.bones:
        ...
        #for g in glueObj.data.collections:
        #        g.unassign(b)

    ## Remove all collections(bone groups) , i commented because it crash blender
    #for g in glueObj.data.collections:
...
def build_rig(rig_class="pos", rotate=False, connect=False):
    ...
    print(
        "use_deform disabled on attachment bones, use the (enable) tool to use them for skinning"
    )
    bpy.ops.object.mode_set(mode="EDIT")
    for b in bpy.context.object.data.edit_bones:
        b.select = False
...
def reset_layer_state(armature):
    ...
    armObj.data.collections[0].is_visible = True

    for i in range(1, len(armObj.data.collections)):
        armObj.data.collections[i].is_visible = False
...
def show_bones(armature=None, group=None, state=True, default=False):
    ...
    if group is None:
        ...
        for boneObj in armObj.data.bones:
            ...
            if (
                bone.startswith("m")
                and bone in skel.avatar_skeleton
                and "Spine" in bone
            ):
                boneObj.hide = not_state
    else:
...
def add_empty_constraints(
    source=None, target=None, transform="COPY_TRANSFORMS", space="WORLD", influence=0
):
    trs_names = {
        "COPY_TRANSFORMS": "ONI TRS",
        "COPY_ROTATION": "ONI ROT",
        "COPY_LOCATION": "ONI LOC",
        "CHILD_OF": "ONI CO",
    }
    if transform not in trs_names:
        print(
            "add_empty_constraint reports: transform",
            transform,
            "is not a key in trs_names, update this later",
        )
    obj = bpy.data.objects
    state = utils.get_state()
                ...
                if modObj.object == armObj:
                    ...
                    if report:
                        print(
                            ...
                            meshObj.name,
                            "has its armature modifier pointing to some other rig",
                        )
...
def create_bone_name(armature=None, bones=[], max=100):
    ...
    for i in range(max):
        ...
        if bone not in armObj.data.bones and bone not in bones:
            return bone
    print(
        "rigs::create_bone_name reports : fall through, apparently there were",
        max,
        "collisions",
    )
    ...
    if all:
        for boneObj in tarmObj.data.edit_bones:
            if boneObj.name in source_x_axis:
                align_bone_x_axis(
                    boneObj, world_to_target_matrix @ source_x_axis[boneObj.name]
                )
            else:
...
def make_complete(
    ...
    if len(bad_bones) == len(armObj.data.bones):
        print(
            "rigsutils::make_complete : None of the bones in the rig matched, there's nothing to do"
        )
                ...
                if skel.avatar_skeleton[bone]["connected"]:
                    if boneObj.parent:
                        ...
                        if pBone.name in skel.avatar_skeleton:
                            ...
                            print(
                                "Missing parent from skel, required for moving tail to head",
                                pBone.name,
                            )
            else:
            ...
            if boneObj.parent:
                if len(boneObj.parent.children) == 1:
                    ...
                    if boneObj.parent.length < 0.01:
                        ...
                        print(
                            "Parent bone tail to head would result in a bone too small, restoring",
                            boneObj.parent.name,
                        )
    ...
    if len(armObj.data.bones) != len(newObj.data.bones):
        ...
        utils.set_state(state)
        print(
            "The imported armature is not compatible with SL, use the dae importer instead."
        )
        print(
            "The imported dae file is not SL compatible and was removed from the scene."
        )
        print(
            "If you are attempting to import and repair a dae file use the Onigiri dae importer instead."
        )
        return False
...
def cycle_rig(armature=None, frame_start=0, frame_end=0):
    ...
    if (
        ...
        or bpy.context.selected_objects[0].name == armObj.name
    ):
        print(
            "It appears that the recycle failed, could not import the saved animation rig:",
            file_path,
        )
        return False
