...
def create_directors(armObj, source=None, move=True):
    ...
    if source == "pool":
        ...
        for boneObj in armObj.pose.bones:
            ...
            bpy.ops.mesh.primitive_cube_add(
                size=size, enter_editmode=False, align="WORLD", location=(0.0, 0.0, 0.0)
            )
            ...
            for modObj in o.modifiers:
                if modObj.type == "ARMATURE":
                    if modObj.object == armObj:
                        ...
                        if vglen == 0:
                            print(
                                "Skipping almost qualified mesh",
                                o.name,
                                "no vertex groups",
                            )
                            break
    ...
    for boneObj in armObj.pose.bones:
        ...
        OBJ["oni_ragdoll_bone_info"] = (
            "I contain the bone name that is that target bone on the actor"
        )
        OBJ["oni_ragdoll_actor"] = armObj
        OBJ["oni_ragdoll_actor_info"] = (
            "I contain the rig object that is the actor containing the target pose bone"
        )
...
def add_constraints(armObj):
    ...
    for boneObj in armObj.pose.bones:
        ...
        if bone not in directors:
            print(
                "missing bone in directors [",
                bone,
                "]",
                "limit probably reached for pool or existing",
                sep="",
            )
            continue
