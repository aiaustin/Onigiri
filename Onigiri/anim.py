...
def save_anim(data=None, file=None):
    ...
    file_content = {}
    file_content["header"] = {}
    file_content["header"]["version"] = (header["version"]).to_bytes(
        2, byteorder="little"
    )
    file_content["header"]["sub_version"] = (header["sub_version"]).to_bytes(
        2, byteorder="little"
    )
    if header["base_priority"] == -1:
        ...
        file_content["header"]["base_priority"] = (header["base_priority"]).to_bytes(
            4, byteorder="little"
        )
    file_content["header"]["duration"] = struct.pack("f", header["duration"])

    file_content["header"]["emote_name"] = struct.pack(
        "%dsB" % len(header["emote_name"]), bytes(header["emote_name"], "utf8"), 0
    )

    file_content["header"]["loop_in_point"] = struct.pack("f", header["loop_in_point"])
    file_content["header"]["loop_out_point"] = struct.pack(
        "f", header["loop_out_point"]
    )
    file_content["header"]["loop"] = (header["loop"]).to_bytes(4, byteorder="little")
    file_content["header"]["ease_in_duration"] = struct.pack(
        "f", header["ease_in_duration"]
    )
    file_content["header"]["ease_out_duration"] = struct.pack(
        "f", header["ease_out_duration"]
    )
    ...
    file_content["header"]["num_joints"] = (header["num_joints"]).to_bytes(
        4, byteorder="little"
    )
    ...
    for bone in joints:
        ...
        file_content["joints"][bone]["priority"] = struct.pack(
            "i", joints[bone]["joint_priority"]
        )

        file_content["joints"][bone]["num_rot_keys"] = joints[bone][
            "num_rot_keys"
        ...
        file_content["joints"][bone]["num_pos_keys"] = joints[bone][
            "num_pos_keys"
        ].to_bytes(4, byteorder="little")
...
def get_rotation_data(
    armature="", bone="", frame="", frame_data="", matrix_type="pose_matrix_basis"
):
