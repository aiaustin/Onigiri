...
def main(
    ...
    if keys:
        if s_has_action:
            ...
            print(
                "The feature (keys) is requested but there's no animation, reverting to spread"
            )
    ...
    s_matrix_data = get_matrices(
        armature=sarmObj.name, start=s_frame_start, end=s_frame_end, force=True
    )
    ...
    if len(bone_list) == 0:
        print(
            "There was no motion available for use.  If this is a result of motion type then set (Keys) as well"
        )
        print(
            "as (Motion) for the capture, currently your settings are as follows: [Keys: ",
            keys,
            "] [Motion: ",
            motion,
            "]",
        ...
        for bone in matrices:
            ...
            for frame in matrices[bone]:
                ...
                if close_enough(rot_now, rot_last, tol=0.01):
                    ...
                    motion[bone][frame - 1]["matrix"] = matrices[bone][frame - 1][
                        "matrix"
                    ]
                    rot_last = rot_now
