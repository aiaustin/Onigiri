...
def export_sl_anim(armature=None, path=None):
    ...
    if anim_start_frame == anim_end_frame:
        print(
            "A frame range of zero was detected, this is non-motion and no content to export"
        )
        popup(
            "Frame range calculates to zero motion, 0 frames, no animation can be exported",
            "Error",
            "ERROR",
        )
        return False
    ...
    if 1 == 0:
        if onia.remove_isolated_keys:
            if has_action:
                ...
                print(
                    "Isolated keys removed:", remove_isolated_keys(armature=armObj.name)
                )
            else:
                print("remove_isolated_keys is enabled but there's no motion data")
    else:
        print(
            "Skipping check for isolated keys, this would damage an animation that was used for poses only"
        )
    ...
    time_slices = get_sl_time(
        total=total_time,
        tpf=time_per_frame,
        start=anim_start_frame,
        stop=anim_end_frame,
    )
    ...
    if not oni.export_avastar_disabled:
        if armObj.get("avastar") is not None and armObj.get("onigiri") is None:
            ...
            print(
                "Frame range - frame_start / frame_end :", frame_start, "/", frame_end
            )
    ...
    if anim_linear:
        ...
        if not has_action:
            ...
            print("")
            print(
                "The armature you're attempting to use as an animation source does not have an action associated with it"
            )
            print(
                "There's nothing to export.  The alternative is to bake the influences to the armature or use (High Fidelity)"
            )
            popup(
                "This method of exporting an animation requires that there is one on the rig, but there isn't.",
                "Error",
                "ERROR",
            )
            return False
        ...
        if anim_use_source_keys:
            ...
            if not has_action:
                ...
                if not lerp_source:
                    ...
                    print(
                        "** lerp returned source key data in lerp_source:", lerp_source
                    )
        ...
        if anim_use_target_keys:
            ...
            lerp_target = get_target_keys(
                armature=armObj.name, frame_start=frame_start, frame_end=frame_end
            )
            if len(lerp_target) == 0:
                print(
                    "There are no usable target armatures where keys can be gathered."
                )
        ...
        if anim_use_source_keys or anim_use_target_keys:
            ...
            if len(lerp_source) == 0 and len(lerp_target) == 0:
                print(
                    "All attempts to gather keys from the source and/or targets was unsuccessful, continuing to raw bake..."
                )
        ...
        if 1 == 0:
            ...
            print(
                ...
                is_avastar,
                "gives me the following bone_data",
            )
            print("=====================================================")
            print("bone_data:")
            print("-----------------------------------------------------")
        ...
        print(
            "This is a programming error, but to fix it please chose an export option."
        )
        print("There are a few, deviations, baking, linear")
        popup(
            "Choose an export option, (Deviation, High Fidelity, Linear)",
            "No Option Set",
            "INFO",
        )
        return False
    ...
    if props["bake_proxy"]:
        print(
            "This is an old bake facility that is obsolete, please report this as a bug"
        )
        popup(
            "You've reach the quiet place, start screaming, or at least report this bug O.o",
            "Error",
            "ERROR",
        )
        return False
        ...
        print(
            "This is a new, disposable, rig that contains the entirety of the animation that would have been exported."
        )
        return True
    ...
    if len(bone_list) == 0:
        print(
            "bone_list is empty so we're assuming we'll get an ordered list from the armature..."
        )
        for boneObj in armObj.data.bones:
            if boneObj.name not in skel.avatar_skeleton:
                print(
                    "Incompatible bone found [", boneObj.name, "] removing...", sep=""
                )
    ...
    if 1 == 0:
        ...
        if onia.disable_pelvis_location_animation:
    ...
    if len(bone_data) == 0:
        print(
            "There was no bone data to write, bone_data is empty.  Turn on (Use Source Keys and/or Use Target Keys)"
        )
        print(
            "This can happen if you have an animation associated with your rig but none of the bones qualify for animation"
        )
        popup(
            "Empty bone data, turn on (use keys), see System Console for details",
            "Empty Bone Data",
            "ERROR",
        )
        anim.export_sl_anim_label_short = anim.export_sl_anim_label
    ...
    if len(bone_list) == 0:
        print(
            "There was no bone list acquired to write, turn on (use keys), bone_list is empty"
        )
        print(
            "This can happen if you have an animation associated with your rig but none of the bones qualify for animation"
        )
        popup(
            "Empty bone list, see System Console for details",
            "Empty Bone List",
            "ERROR",
        )
        anim.export_sl_anim_label_short = anim.export_sl_anim_label
...
def write_animation(
    ...
    file_content["header"]["joint_count"] = (int(len(bone_list))).to_bytes(
        4, byteorder="little"
    )
    ...
    for bone in bone_list:
        ...
        if bone_data[bone].get("rot"):
            ...
            rot_count = len(rots_)
            file_content["joints"][bone]["rot_count"] = rot_count.to_bytes(
                4, byteorder="little"
            )
        ...
        if bone_data[bone].get("loc"):
            ...
            loc_count = len(locs_)
            file_content["joints"][bone]["loc_count"] = loc_count.to_bytes(
                4, byteorder="little"
            )
    ...
    if file_length > 250000:
        ...
        print(txt)
        utils.popup(
            "File is too large for Second Life, exported anyway but see console for instructions.",
            "Info",
            "INFO",
        )
        return False
...
def write_mapped_animation(
    ...
    for bone in bone_list:
        ...
        if armObj.pose.bones[sbone].get("priority_enabled") == 1:
            if armObj.pose.bones[sbone].get("priority") is not None:
...
def get_baked_animation(
    ...
    if mark_tol:
        ...
        for bone in motion:
            ...
            for frame in frames:
                ...
                loc_mark = False
                for i in range(3):
                    if _ce(rot_start[i], rot[i], tol=mrot):
                        rot_mark = True
                        animated.add(bone)
                        break
                    if _ce(loc_start[i], loc[i], tol=mloc):
                        loc_mark = True
                        animated.add(bone)
                        break
    ...
    if len(animated) == 0:
        print(
            "No animated bones were detected when attempting to bake high fidelity motion"
        )
        return False
    ...
    keep_rots = set()
    keep_locs = set()
    if oni_anim.anim_resample:
        print("resampling baked animation")
        r = [
            i
            for i in range(
                frame_start, frame_end + 1, oni_anim.anim_resample_rate_rotation
            )
        ]
        l = [
            i
            for i in range(
                frame_start, frame_end + 1, oni_anim.anim_resample_rate_location
            )
        ]
        keep_rots = set(r)
        keep_locs = set(l)
    ...
    if onia.disable_pelvis_location_animation:
        ...
        for bone in bones_del:
            print(
                "disable_pelvis_location_animation is True, removing root from location data:",
                bone,
            )
            stage_one.pop(bone, "")
    ...
    if onia.disable_location_offsets:
        ...
        for bone in bones_del:
            print(
                "disable_location_offsets is True, removing location data from bone:",
                bone,
            )
            stage_one.pop(bone, "")
...
def get_animated_frames(
    ...
    for fc in fcurves:
        ...
        if transform_type == "rotation_quaternion" and rot_mode == "QUATERNION":
            ...
            print(
                "There is no logical way to proceed, you have to change the affected rig bones rotation mode"
            )
            print(
                "manually or retarget the offending animation to a new rig, make sure the new rig has only"
            )
            print("quaternion rotation modes on the bones.")
            print("--")
            print(
                "Potentially you can convert the animation using one of Onigiri's tools, from Euler to"
            )
            print(
                "quaternion, but you will still need to make sure that the bones are compliant.  It's expected"
            )
            print(
                "that this is a Onigiri rig so just generate a new rig to test the results."
            )
            loc_rot = "rot"
...
def get_animated_keys(armature=None, frame_start=0, frame_end=0):
    oni = bpy.context.scene.onigiri
    onia = bpy.context.scene.oni_anim_props
    oni_anim = bpy.context.scene.oni_anim
    ...
    for fc in fcurves:
        ...
        if transform_type == "rotation_quaternion" and rot_mode == "QUATERNION":
            ...
            elif onia.disable_location_offsets:
                continue
    ...
    if frame_error:
        ...
        print(
            "Either your frame range is wrong or the action associated with this armature,",
            armObj.name,
        )
        print(
            "have different rotation modes, eg: quaternion vs Euler, please correct the issue on your rig."
        )

    return frame_data


def get_source_keys(armature=None, frame_start=0, frame_end=0):
    oni = bpy.context.scene.onigiri
    onia = bpy.context.scene.oni_anim_props
    anim = bpy.context.scene.oni_anim
    oni_anim = bpy.context.scene.oni_anim
    ...
    for fc in fcurves:
        ...
        if (
            ...
            or transform_type == "rotation_euler"
        ):
            if use_rotation:
                loc_rot = "rots"
        elif transform_type == "location":
...
def get_target_keys(
    ...
    for boneObj in armObj.pose.bones:
        ...
        for cObj in boneObj.constraints:
            if cObj.type in constraint_types:
                if cObj.target is None:
                    continue

                tarmObj = cObj.target

                tarmReal = armObj.get("oni_motion_director")
                if tarmReal is not None:
    ...
    for sbone in bone_targets:
        for tarm in bone_targets[sbone]:
            ...
            if obj[tarm].animation_data is None:
                continue

            if obj[tarm].animation_data.action is None:
                continue
    ...
    for fc in fcurves:
        ...
        if transform_type == "rotation_quaternion" and rot_mode == "QUATERNION":
            ...
            if not been_here:
                ...
                print(
                    "The attempt to generate the keys requested will continue but will probably have animation anomalies."
                )
                been_here = True
            lrs = "rot"
...
def clean_motion(matrices=None, rot_tol=1.0, loc_tol=0.01):
    ...
    for bone in matrices:
        ...
        for frame in matrices[bone]:
            ...
            for r in range(3):
                mark = False
                if close_enough(rot[r], last_rot[r], tol=eu_tol):
            ...
            for l in range(3):
                mark = False
                if close_enough(loc[l], last_loc[l], tol=loc_tol):
                    mark = True
...
def get_deviations(
    ...
    if mark_tol:
        ...
        for bone in matrices:
            ...
            for frame in range(1, len(frames)):
                ...
                loc_mark = False
                for i in range(3):
                    if close_enough(rot_start[i], rot[i], tol=mrot):
                        rot_mark = True
                        animated.add(bone)
                        break
                    if close_enough(loc_start[i], loc[i], tol=mloc):
                        loc_mark = True
                        animated.add(bone)
                        break
                if rot_mark or loc_mark:
    ...
    for bone in lerp_data:
        ...
        bone_data[bone] = {}
        if lerp_data[bone].get("rots") is not None:
            ...
            for frame in frames:
                ...
                if bone_data[bone].get("rot") is None:
                    bone_data[bone]["rot"] = {}
        ...
        if lerp_data[bone].get("locs") is not None:
            ...
            for frame in frames:
                ...
                if bone_data[bone].get("loc") is None:
                    bone_data[bone]["loc"] = {}
    ...
    if 1 == 0:
        key_proxy_rig(
            armature=armObj.name, matrices=matrices, lerp_data=lerp_data, flatten=False
        )
...
def get_motion(armature=None, frame_start=0, frame_end=0, frames=[], bones=[]):
    ...
    print(
        "animutils::get_motion : frame_start / frame end:", frame_start, "/", frame_end
    )
    ...
    if abs(frame_start - frame_end) == 0 and len(frames) == 0:
        print(
            "get_motion reports: no frames to examine, this is an internal error probably, call the doctor!"
        )
        return False
...
def get_bone_data(
    armature=None, lerp_data=None, motion=None, time_slices=None, is_avastar=False
):
...
def calculate_time(
    ...
    if 1 == 0:
        if anim.anim_loop:
            txt = ""
            if oni_anim.anim_loop_advanced:
            ...
            if txt != "":
                print(txt)
                print(
                    "The loop failure described above must be corrected or your animation will not loop and may not even play"
                )
                print(
                    "There could be other loop issues that will present themselves after you correct the one above"
                )
...
def recycle_actions():
    ...
    for a in actions:
        if oni_alib["actions"].get(a) is None:
            oni_alib["actions"][a] = {}
...
def get_key_stops(
    ...
    for bone in bones:
        ...
        for frame in range(frame_start, frame_end + 1):
            ...
            for i in range(3):
                rot_start_axis, rot_now_axis = _round(rot_start[i], 4), _round(
                    rot_now[i], 4
                )

                if _ce(rot_start_axis, rot_now_axis, tol=mark_tol_rot):
                    rot_mark = True
            ...
            for i in range(3):
                loc_start_axis, loc_now_axis = _round(loc_start[i], 4), _round(
                    loc_now[i], 4
                )
                if _ce(loc_start_axis, loc_now_axis, tol=mark_tol_loc):
                    loc_mark = True
...
def clean_frames(
    ...
    for frame in frames_list:
        ...
        for i in range(3):
            if _ce(tr_start[i], tr_now[i], tol=tol):
...
def convert_animated_controllers(source=None, target=None):
    ...
    if avaRig.animation_data is None:
        print("convert_animated_controllers reports: no animation_data")
        return False
    if avaRig.animation_data.action is None:
...
def morph_to(source=None, target=None, morph=False):
    ...
    if sourceObj.animation_data is None:
        sourceObj.animation_data_create()
    if sourceObj.animation_data.action is None:
...
def bake_motion(sarm=None, tarm=None, frame_start=0, frame_end=0):
    ...
    if sarm is None or tarm is None:
        ...
        for fc in srcAnim.fcurves:
            ...
            if 1 == 1:
                ...
                for kfp in fc.keyframe_points:
                    ...
                    kfp_data[fc.data_path][fc.array_index]["handle_left"].append(
                        kfp.handle_left.copy()
                    )
                    kfp_data[fc.data_path][fc.array_index]["handle_left_type"].append(
                        kfp.handle_left_type
                    )
                    kfp_data[fc.data_path][fc.array_index]["handle_right"].append(
                        kfp.handle_right.copy()
                    )
                    kfp_data[fc.data_path][fc.array_index]["handle_right_type"].append(
                        kfp.handle_right_type
                    )
                    kfp_data[fc.data_path][fc.array_index]["interpolation"].append(
                        kfp.interpolation
                    )
    ...
    for fc in animObj.fcurves:
        ...
        if fc.data_path in kfp_data:
            ...
            for kfp in fc.keyframe_points:
                ...
                kfp.handle_left = kfp_data[fc.data_path][fc.array_index]["handle_left"][
                    index
                ]
                kfp.handle_left_type = kfp_data[fc.data_path][fc.array_index][
                    "handle_left_type"
                ][index]
                kfp.handle_right = kfp_data[fc.data_path][fc.array_index][
                    "handle_right"
                ][index]
                kfp.handle_right_type = kfp_data[fc.data_path][fc.array_index][
                    "handle_right_type"
                ][index]
                kfp.interpolation = kfp_data[fc.data_path][fc.array_index][
                    "interpolation"
                ][index]
                ...
                if 1 == 0:
                    if "mShoulderLeft" in fc.data_path:
                        ...
                        print(
                            " -save x/y:",
                            kfp_data[fc.data_path][fc.array_index]["co.x"][index],
                            "/",
                            kfp_data[fc.data_path][fc.array_index]["co.y"][index],
                        )
...
def split_time(frame_start=0, frame_end=0, fill_fps=24, fill_time=60):
    ...
    if total_time <= fill_time:
        print(
            "The custom fill time accommodates the entire animation, splitting is not required."
        )
        print(
...
def write_lsl(source=None, target=None, actions=None, prefix="Anim", fps=24):
    ...
    for action in actions:
        ...
        if final_time < 0:
            print(
                "final_time is less than 0, skipping calculation [action/time]:",
                action,
                "/",
                final_time,
            )
