...
view_layers_order["avastar"] = [
    ...
    "SL Base (deform)",
    "Volume (deform)",
    "Bento Bones -Hands (deform)",
    "Bento Bones -Hinds (deform)",
    "Bento Bones -Face (deform)",
    "Bento Bones -Wings (deform)",
    "Bento Bones -Spine (deform)",
    "Bento Bones -Tail (deform)",
    "Bento Bones -Groin (deform)",
]
...
def see_all():
    ...
    for vlObj in bpy.context.scene.view_layers:
        ...
        bpy.context.window.view_layer = bpy.data.scenes[scene_name].view_layers[
            view_layer
        ]
...
def save_state():
    ...
    for c in bpy.data.collections:
        ...
        oniv["states"]["data"][ID]["collections"][c.name][
            "hide_viewport"
        ] = c.hide_viewport
    ...
    for vlObj in bpy.context.scene.view_layers:
        ...
        oniv["states"]["data"][ID]["layers"][view_layer]["selections"] = [
        ...
        for colObj in vlObj.layer_collection.children:
            cname = colObj.name
            oniv["states"]["data"][ID]["layers"][view_layer]["collections"][
                cname
            ] = dict()
            oniv["states"]["data"][ID]["layers"][view_layer]["collections"][cname][
                "hide_viewport"
            ] = vlObj.layer_collection.children[cname].hide_viewport
            vlObj.layer_collection.children[cname].hide_viewport = False

        for o in bpy.data.objects:
            oniv["states"]["data"][ID]["layers"][view_layer]["objects"][
                o.name
            ] = o.hide_get()
            o.hide_set(False)
    ...
    for o in bpy.data.objects:
        ...
        for boneObj in o.data.bones:
            bname = boneObj.name
            oniv["states"]["data"][ID]["armatures"][o.name]["pose_hide"][
                bname
            ] = boneObj.hide
            oniv["states"]["data"][ID]["armatures"][o.name]["hide_select"][
                bname
            ] = boneObj.hide_select
            boneObj.hide = False
        ...
        for boneObj in o.data.edit_bones:
            bname = boneObj.name
            oniv["states"]["data"][ID]["armatures"][o.name]["edit_hide"][
                bname
            ] = boneObj.hide
            boneObj.hide = False
        bpy.ops.object.mode_set(mode="OBJECT")
...
def restore_state():
    ...
    for oname in oniv["states"]["data"][ID]["armatures"]:
        ...
        for bname in oniv["states"]["data"][ID]["armatures"][oname]["pose_hide"]:
            if bname in obj[oname].data.bones:
                obj[oname].data.bones[bname].hide = oniv["states"]["data"][ID][
                    "armatures"
                ][oname]["pose_hide"][bname]
                obj[oname].data.bones[bname].hide_select = oniv["states"]["data"][ID][
                    "armatures"
                ][oname]["hide_select"][bname]

        bpy.ops.object.mode_set(mode="EDIT")
        for bname in oniv["states"]["data"][ID]["armatures"][oname]["edit_hide"]:
            if bname in obj[oname].data.edit_bones:
                obj[oname].data.edit_bones[bname].hide = oniv["states"]["data"][ID][
                    "armatures"
                ][oname]["edit_hide"][bname]
        bpy.ops.object.mode_set(mode="OBJECT")
        obj[oname].select_set(False)

    for cname in oniv["states"]["data"][ID]["collections"]:
        if cname in bpy.data.collections:
            bpy.data.collections[cname].hide_viewport = oniv["states"]["data"][ID][
                "collections"
            ][cname]["hide_viewport"]
            bpy.data.collections[cname].hide_select = oniv["states"]["data"][ID][
                "collections"
            ][cname]["hide_select"]
        else:
    ...
    for oname in oniv["states"]["data"][ID]["objects"]:
        if oname in obj:
            obj[oname].hide_select = oniv["states"]["data"][ID]["objects"][oname][
                "hide_select"
            ]
            obj[oname].hide_viewport = oniv["states"]["data"][ID]["objects"][oname][
                "hide_viewport"
            ]
        else:
    ...
    for view_layer in oniv["states"]["data"][ID]["layers"]:
        ...
        for cname in oniv["states"]["data"][ID]["layers"][view_layer]["collections"]:
            vlObj.layer_collection.children[cname].hide_viewport = oniv["states"][
                "data"
            ][ID]["layers"][view_layer]["collections"][cname]["hide_viewport"]

        for oname in oniv["states"]["data"][ID]["layers"][view_layer]["objects"]:
            if oname not in obj:
                print("object missing in scene:", oname)
                continue
            hide_state = oniv["states"]["data"][ID]["layers"][view_layer]["objects"][
                oname
            ]
...
class OnigiriPanelView(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        box = layout.box()
        box.label(text="Rig View Options: " + oni_view.rig_author_name)
        ...
        if 1 == 0:
            row.prop(oni_view, "dummy", toggle=True, text="Volume Bones")
            row.prop(oni_view, "dummy", toggle=True, text="Non-Human Bones")
            row = col.row(align=True)
            row.prop(oni_view, "dummy", toggle=True, text="Face Bones")
            row.prop(oni_view, "dummy", toggle=True, text="Hand Bones")
