...
class OnigiriTemplateOneMapProperties(bpy.types.PropertyGroup):
    ...
    onemap_reskin: bpy.props.BoolProperty(
        ...
        "\n\n"
        "While this is enabled you choose a number of bones that you would like to remove and have their mesh weights assigned to the "
        "bone that you chose when you enabled this mode.  This allows you to smoothly merge vertex weights from a bone that can't be "
        "used in the target system to a bone that can be used.",
        default=False,
...
class OnigiriTemplateLoadOneMap(bpy.types.Operator, ImportHelper):
    ...
    filter_glob: bpy.props.StringProperty(default="*.ctm;*.ccm", options={"HIDDEN"})
