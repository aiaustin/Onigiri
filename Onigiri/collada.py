...
def write_collada(
    ...
    if export_path_to_pelvis:
        print(
            "export_path_to_pelvis is designed to give you proper deformations while still being compatible with other applications."
        )
    if export_full_rig:
        print(
    ...
    if use_app_compatible_data and armObj.get("oni_devkit_transforms") is not None:
        ...
        for bone in armObj["oni_devkit_transforms"]["bind_pose"]:
            bind_pose[bone] = mathutils.Matrix(
                armObj["oni_devkit_transforms"]["bind_pose"][bone]
            )
        joint_pose = {}
        for bone in armObj["oni_devkit_transforms"]["joint_pose"]:
            joint_pose[bone] = mathutils.Matrix(
                armObj["oni_devkit_transforms"]["joint_pose"][bone]
            )
    ...
    if use_rig_data:
        ...
        if rig_class != "":
            ...
            if not utils.can_select(object=armObj.name):
                ...
                popup(
                    "Export failed, wrong export option?  Where is the rig?",
                    "Error",
                    "ERROR",
                )
                return False
        ...
        if len(not_bind_info) > 0:
            print(
                "The following bones did not have imported bind info so the rig/bone was used instead:"
            )
            print(not_bind_info)
            if len(has_bind_info) > 0:
                print(
                    "The following bones had bind info and were processed using that available data:"
                )
                print(has_bind_info)
            else:
                print(
                    "There was no imported bind information available for any of the bones"
                )
        ...
        print(
            "WARNING: use_offset_location for bind pose defaults to False when using Avastar pose"
        )
        ...
        for boneObj in armObj.data.bones:
            ...
            if dBone.parent and dBone.parent.use_deform:
                ...
                print(
                    "Storing global to local with questionable orientation for ROOT bone"
                )
...
def edit_dae(
                ...
                if data.tag == n + "float_array":
                    ...
                    if param.get("type") == "float4x4":
                        ...
                        if export_full_rig:
                            ...
                            all_bones = [
                                b for b in skel.avatar_skeleton if " " not in b
                            ]
                        ...
                        if export_path_to_pelvis:
                            ...
                            for bone in skel.avatar_skeleton:
                                ...
                                converted_names[new_bone]["parent"] = (
                                    skel.avatar_skeleton[bone]["parent"]
                                )
    ...
    if write_nodes:
        ...
        if len(missing_joints) > 0:
            print("edit_dae:", "Some joints have no associated data.")
...
def fix_nodes(root=None):
    ...
    print(
        "WARNING: collada::fix_nodes reports: does not provide rotation data, fix later"
    )
...
def export_custom(
    ...
    if not utils.can_select(object=armObj.name):
        ...
        popup(
            "Export failed, wrong export option?  Where is the rig?", "Error", "ERROR"
        )
        return False
                    ...
                    if pbone not in transforms["bone_data"]:
                        if report:
                            print(
                                ...
                                "is a parent of",
                                cbone,
                                "and requires a joint matrix but doesn't exist.",
                            )
...
def build_dae_matrices(armature=None, base="female_neutral", report=False):
    ...
    if armObj is None:
        ...
        txt = (
            "collada::build_dae_matrices : armature provided, will fill missing with "
            + base
        )
    print(txt)
